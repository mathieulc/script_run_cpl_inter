...
sed -e "s/<yr1>/${YEAR_BEGIN_JOB}/g"  -e "s/<mo1>/${ms}/g"  -e "s/<dy1>/${ds}/g"  -e "s/<hr1>/00/g"  \
    ...
    -e "s/<wavdt>/${TSP_WW3}/g" \
    ${WW3_IN_DIR}/inputs_ww3/ww3_shel.inp.base.${RUN} > ./ww3_shel.inp
...
for k in `seq 0 $(( ${lengthforc} - 1))` ; do
    ...
    echo "cp -f ${WW3_IN_DIR}/ww3_prnc.inp.${forcww3[$k]} ./"
    cpfile ${WW3_IN_DIR}/inputs_ww3/ww3_prnc.inp.${forcww3[$k]} ./
 done
